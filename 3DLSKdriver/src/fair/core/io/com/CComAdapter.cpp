...
void CComAdapter::init() {
	...
	if (strncmp("/dev/ttyUSB", _sDeviceName.c_str(), 11) == 0)
		snprintf(cmd, sizeof(cmd), "setserial %s divisor 48 spd_cust",
				_sDeviceName.c_str());
	else {
		if (strncmp("/dev/ttyS", _sDeviceName.c_str(), 9) == 0)
			snprintf(cmd, sizeof(cmd),
					"setserial %s baud_base 500000 spd_cust divisor 1",
					_sDeviceName.c_str());
		else
...
unsigned long CComAdapter::receiveUnbuffered(char* szAnswer,
...
unsigned long CComAdapter::receiveBuffered(char* szAnswer,
		unsigned long ulLength) {
	if (_hCom == 0)
		return 0;
...
/**
 * To fix the ringindicator error (switch of pin 6 and 9) do it here
 */
bool CComAdapter::getRingIndicator() {
	int signalState;
	//	cout << "in getRingIndicator" << endl;
	ioctl(_hCom, TIOCMIWAIT, TIOCM_RNG);
